...
namespace Magic.Compiler2
{
    /// <summary>
    /// Magic compiler 2.0 CLI entry point.
    /// <para>
    /// Improvements over v1.0 (<c>spc</c>):
    /// </para>
    /// <list type="bullet">
    /// <item>Uses <see cref="Compiler2"/> which builds a <strong>complete, fully-typed AST</strong> during parsing.</item>
    /// <item><see cref="Magic.Kernel2.Compilation2.SemanticAnalyzer2"/> walks typed AST nodes — no deferred lowering.</item>
    /// <item><see cref="Assembler2"/> generates bytecode directly from the AST — no <c>StatementLoweringCompiler</c>.</item>
    /// </list>
    /// </summary>
    class Program
    {
        ...
        static async Task<int> Main(string[] args)
        {
            ...
            try
            {
                ...
                var compiler = new Magic.Kernel2.Compilation2.Compiler2();
                ...
                if (useAnim)
                {
                    ...
                    spinnerTask = ShowSpinnerAsync(cts.Token, "Compiling [v2.0]");
                }
                else
                {
                    PrintInfo("Compiling...");
        ...
        static void PrintHeader()
        {
            ...
            Console.Write("║");
            Console.ForegroundColor = ConsoleColor.Cyan;
            Console.Write("  Magic: Meta AGI Compiler v.2.0".PadRight(57));
            Console.ForegroundColor = ConsoleColor.Magenta;
            Console.WriteLine("║");
            Console.Write("║");
            Console.ForegroundColor = ConsoleColor.DarkCyan;
            Console.Write("  Full AST parse · No lowering · Direct AST→ASM".PadRight(57));
            Console.ForegroundColor = ConsoleColor.Magenta;
            Console.WriteLine("║");
            Console.WriteLine("╚═══════════════════════════════════════════════════════════╝");
            Console.ResetColor();
        ...
        static void PrintUsage()
        {
            ...
            Console.WriteLine("Usage: Magic.Compiler2 [options] <file.agi>");
            Console.WriteLine("       Compiles .agi file using v2.0 pipeline (full AST, no lowering)");
            Console.WriteLine("Options:");
            Console.WriteLine("       --output agiasm|agic, -o agiasm|agic   Output format (default: agic)");
            Console.WriteLine("       --no-anim   Disable compile animation");
            Console.ResetColor();
        }
