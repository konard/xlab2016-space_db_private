...
/// <summary>Подсветка .agi по токенам Scanner (тот же лексер, что и компилятор).</summary>
internal sealed class MagicAgiColorizer : DocumentColorizingTransformer
{
    private static readonly IBrush PlainBrush = CreateBrush(0xDD, 0xE8, 0xFF);
    private static readonly IBrush KeywordBrush = CreateBrush(0x6C, 0x9E, 0xFF);
    private static readonly IBrush IdentifierBrush = CreateBrush(0xA8, 0xC5, 0xFF);
    private static readonly IBrush DeclNameBrush = CreateBrush(0x9A, 0xD8, 0xE8);
    private static readonly IBrush TypeBrush = CreateBrush(0xE5, 0xB6, 0x5C);
    private static readonly IBrush StringBrush = CreateBrush(0x7E, 0xDC, 0x9A);
    private static readonly IBrush NumberBrush = CreateBrush(0xF5, 0xA8, 0x7A);
    private static readonly IBrush CommentBrush = CreateBrush(0x6B, 0x7A, 0x99);
    private static readonly IBrush PunctuationBrush = CreateBrush(0x9A, 0xA8, 0xCC);
    ...
    private IReadOnlyList<(int start, int end, IBrush brush)> _spans = [];
    ...
    public void DocumentUpdated(TextDocument document)
    {
        ...
        var list = new List<(int, int, IBrush)>(tokenList.Count);
    ...
    private static IBrush BrushFor(Token t, bool isTypeIdentifier, bool isDeclName)
    {
