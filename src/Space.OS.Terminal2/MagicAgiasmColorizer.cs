...
{
    private static readonly IBrush PlainBrush = CreateBrush(0xC8, 0xE0, 0xD0);
    ...
    public void DocumentUpdated(TextDocument document)
    {
        ...
        var list = new List<(int, int, IBrush)>();
        var lineStart = 0;
    ...
    private static void AddSpansForLine(string text, int lineOffset, int lineLen, List<(int, int, IBrush)> list)
    {
        if (lineLen <= 0)
            return;
    ...
    private static void TokenizeOperands(string text, int start, int end, List<(int, int, IBrush)> list)
    {
        var i = start;
        while (i < end)
        {
